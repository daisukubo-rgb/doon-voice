...
function MainApp() {
  ...
  async function openSelectionQuestion() {
    ...
    try {
      ...
      try {
        ...
        if (operation === questionOperationRef.current) setNotice(errorMessage(error, "選択した文章または画面を読み取れませんでした"));
      }
    }
  }
  ...
  async function applySelectionQuestionShortcut(next: string, notify = true) {
    ...
    try {
      ...
      if (notify) setNotice(`選択文・画面を質問するキーを ${shortcutLabel(next, navigator.userAgent.includes("Mac"))} に変更しました`);
    } catch {
      if (operation !== selectionQuestionShortcutOperationRef.current) return;
