...
AIによる変更を自動採用するのは「、」「。」だけです。語句・否定・数字・記号の変更や文章の欠落を検出した場合、原文を使います。URLやメールアドレスを含む文も原文を使います。認識ミスの自由な書き換えは行いません。AIで整えた文章に「1つ目」「2つ目」のような連番が二つ以上ある場合は、語句を変えずに箇条書きとして見やすく表示します。

ホームの `選択した文章を質問` は、質問したい文章を選択してから押します。アクセシビリティを許可済みなら選択文を自動で取得します。許可していない場合は、コピーしてから押します。選択文を確認し、質問を入力するか `音声で質問` で話します。回答はポップアップに表示され、`回答をコピー` または `カーソル位置へ入力` を選べます。質問の本文と選択文は、選んだAIへ送ります。AIなしでは質問に答えられません。 ほかのアプリで文章を選択したまま開始・停止キーで質問を話すと、ボタンを押さずに回答ポップアップを開きます。選択がない場合は通常の音声入力です。
