...
起動時にGitHubから最新版を取得する機能はありません。更新する場合は新しいソースを取得するか、完成版インストーラーを入れ直してください。ソース版の起動は、毎回開発用のビルド環境を使います。
...
`npm run setup` はNode.js 20以上、Rust、OSごとのビルド・表示環境を確認し、npmの依存関係をインストールします。確認だけ行う場合は `npm run doctor` を使います。
...
GitHub Releasesには、各インストーラーを入れたZIPも添付します。ZIPを展開し、中のDMG（macOS）またはMSI（Windows）をダブルクリックしてください。配布ZIPには、使い方の `README.txt`、プライバシー説明、OSS表示と `licenses` フォルダも同梱しています。
...
接続と設定の `ログインする` から対応CLIのログインを始めます。CLIが未導入の場合は、案内に従って先に準備してください。ローカルAIを使う場合は `Ollamaを自動インストール`、続けて `Gemmaを取得` を選んでください。ローカルモデル（約7.2GB）は初回だけ取得し、配布物には含めません。
...
AIによる変更を自動採用するのは「、」「。」だけです。語句・否定・数字・記号の変更や文章の欠落を検出した場合、原文を使います。URLやメールアドレスを含む文も原文を使います。認識ミスの自由な書き換えは行いません。

マイク切断や処理中止などで途中までの文章が得られた場合は、確認してからコピーできます。この文章は再試行しても自動で貼り付けません。回収が必要な結果は、コピーまたは破棄するまで次の録音で上書きしません。画面の結果はアプリ終了時に失われます。
...
- ChatGPT / Claude / Geminiを選んだ場合、整形する本文と登録辞書を各対応CLIへ渡します。CLI側の設定・履歴・送信後の保存は、利用者の設定と各提供者の仕組みに従います。
- ローカルAIを選んだ場合は `127.0.0.1` のOllamaだけに本文と辞書を渡します。
